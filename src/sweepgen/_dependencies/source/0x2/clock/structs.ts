...
export class Clock implements StructClass { __StructClass = true as const;
 ...
 id: UID.bcs, timestampMs: bcs.u64()

}) };

 static fromFields( fields: Record<string, any> ): Clock { return Clock.reified( ).new( { id: decodeFromFields(UID.reified(), fields.id), timestampMs: decodeFromFields("u64", fields.timestampMs) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Clock { if (!isClock(item.type)) { throw new Error("not a Clock type");

 }

 return Clock.reified( ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id), timestampMs: decodeFromFieldsWithTypes("u64", item.fields.timestampMs) } ) }

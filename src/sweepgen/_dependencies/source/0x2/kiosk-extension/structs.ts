...
export function isExtension(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V25}::kiosk_extension::Extension`; }
...
export class Extension implements StructClass { __StructClass = true as const;
 ...
 storage: Bag.bcs, permissions: bcs.u128(), isEnabled: bcs.bool()

}) };

 static fromFields( fields: Record<string, any> ): Extension { return Extension.reified( ).new( { storage: decodeFromFields(Bag.reified(), fields.storage), permissions: decodeFromFields("u128", fields.permissions), isEnabled: decodeFromFields("bool", fields.isEnabled) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Extension { if (!isExtension(item.type)) { throw new Error("not a Extension type");

 }

 return Extension.reified( ).new( { storage: decodeFromFieldsWithTypes(Bag.reified(), item.fields.storage), permissions: decodeFromFieldsWithTypes("u128", item.fields.permissions), isEnabled: decodeFromFieldsWithTypes("bool", item.fields.isEnabled) } ) }
...
export function isExtensionKey(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V25}::kiosk_extension::ExtensionKey` + '<'); }
...
export class ExtensionKey<Ext extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V25}::kiosk_extension::ExtensionKey`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = ExtensionKey.$typeName; readonly $fullTypeName: `${typeof PKG_V25}::kiosk_extension::ExtensionKey<${PhantomToTypeStr<Ext>}>`; readonly $typeArgs: [PhantomToTypeStr<Ext>]; readonly $isPhantom = ExtensionKey.$isPhantom;

 readonly dummyField: ToField<"bool">

 private constructor(typeArgs: [PhantomToTypeStr<Ext>], fields: ExtensionKeyFields<Ext>, ) { this.$fullTypeName = composeSuiType( ExtensionKey.$typeName, ...typeArgs ) as `${typeof PKG_V25}::kiosk_extension::ExtensionKey<${PhantomToTypeStr<Ext>}>`; this.$typeArgs = typeArgs;
 ...
 static fromFields<Ext extends PhantomReified<PhantomTypeArgument>>( typeArg: Ext, fields: Record<string, any> ): ExtensionKey<ToPhantomTypeArgument<Ext>> { return ExtensionKey.reified( typeArg, ).new( { dummyField: decodeFromFields("bool", fields.dummyField) } ) }
 ...
 return ExtensionKey.reified( typeArg, ).new( { dummyField: decodeFromFieldsWithTypes("bool", item.fields.dummyField) } ) }

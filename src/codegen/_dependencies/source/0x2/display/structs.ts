...
export function isDisplay(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V25}::display::Display` + '<'); }
...
export class Display<T extends PhantomTypeArgument> implements StructClass { static readonly $typeName = `${PKG_V25}::display::Display`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = Display.$typeName; readonly $fullTypeName: `${typeof PKG_V25}::display::Display<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = Display.$isPhantom;

 readonly id: ToField<UID>; readonly fields: ToField<VecMap<String, String>>; readonly version: ToField<"u16">

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: DisplayFields<T>, ) { this.$fullTypeName = composeSuiType( Display.$typeName, ...typeArgs ) as `${typeof PKG_V25}::display::Display<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id;; this.fields = fields.fields;; this.version = fields.version; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): DisplayReified<ToPhantomTypeArgument<T>> { return { typeName: Display.$typeName, fullTypeName: composeSuiType( Display.$typeName, ...[extractType(T)] ) as `${typeof PKG_V25}::display::Display<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: Display.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => Display.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Display.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => Display.fromBcs( T, data, ), bcs: Display.bcs, fromJSONField: (field: any) => Display.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => Display.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => Display.fromSuiParsedData( T, content, ), fetch: async (client: SuiClient, id: string) => Display.fetch( client, T, id, ), new: ( fields: DisplayFields<ToPhantomTypeArgument<T>>, ) => { return new Display( [extractType(T)], fields ) }, kind: "StructClassReified", } }
...
export function isDisplayCreated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V25}::display::DisplayCreated` + '<'); }
...
export class DisplayCreated<T extends PhantomTypeArgument> implements StructClass { static readonly $typeName = `${PKG_V25}::display::DisplayCreated`; static readonly $numTypeParams = 1; static readonly $isPhantom = [true,] as const;

 readonly $typeName = DisplayCreated.$typeName; readonly $fullTypeName: `${typeof PKG_V25}::display::DisplayCreated<${PhantomToTypeStr<T>}>`; readonly $typeArgs: [PhantomToTypeStr<T>]; readonly $isPhantom = DisplayCreated.$isPhantom;

 readonly id: ToField<ID>

 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: DisplayCreatedFields<T>, ) { this.$fullTypeName = composeSuiType( DisplayCreated.$typeName, ...typeArgs ) as `${typeof PKG_V25}::display::DisplayCreated<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;

 this.id = fields.id; }

 static reified<T extends PhantomReified<PhantomTypeArgument>>( T: T ): DisplayCreatedReified<ToPhantomTypeArgument<T>> { return { typeName: DisplayCreated.$typeName, fullTypeName: composeSuiType( DisplayCreated.$typeName, ...[extractType(T)] ) as `${typeof PKG_V25}::display::DisplayCreated<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`, typeArgs: [ extractType(T) ] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>], isPhantom: DisplayCreated.$isPhantom, reifiedTypeArgs: [T], fromFields: (fields: Record<string, any>) => DisplayCreated.fromFields( T, fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => DisplayCreated.fromFieldsWithTypes( T, item, ), fromBcs: (data: Uint8Array) => DisplayCreated.fromBcs( T, data, ), bcs: DisplayCreated.bcs, fromJSONField: (field: any) => DisplayCreated.fromJSONField( T, field, ), fromJSON: (json: Record<string, any>) => DisplayCreated.fromJSON( T, json, ), fromSuiParsedData: (content: SuiParsedData) => DisplayCreated.fromSuiParsedData( T, content, ), fetch: async (client: SuiClient, id: string) => DisplayCreated.fetch( client, T, id, ), new: ( fields: DisplayCreatedFields<ToPhantomTypeArgument<T>>, ) => { return new DisplayCreated( [extractType(T)], fields ) }, kind: "StructClassReified", } }
...
export function isVersionUpdated(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V25}::display::VersionUpdated` + '<'); }

...
export function isMiner(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::miner::Miner`; }
...
export class Miner implements StructClass { static readonly $typeName = `${PKG_V1}::miner::Miner`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = Miner.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::miner::Miner`; readonly $typeArgs: []; readonly $isPhantom = Miner.$isPhantom;
 ...
 private constructor(typeArgs: [], fields: MinerFields, ) { this.$fullTypeName = composeSuiType( Miner.$typeName, ...typeArgs ) as `${typeof PKG_V1}::miner::Miner`; this.$typeArgs = typeArgs;
 ...
 static reified( ): MinerReified { return { typeName: Miner.$typeName, fullTypeName: composeSuiType( Miner.$typeName, ...[] ) as `${typeof PKG_V1}::miner::Miner`, typeArgs: [ ] as [], isPhantom: Miner.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Miner.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Miner.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Miner.fromBcs( data, ), bcs: Miner.bcs, fromJSONField: (field: any) => Miner.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Miner.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Miner.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => Miner.fetch( client, id, ), new: ( fields: MinerFields, ) => { return new Miner( [], fields ) }, kind: "StructClassReified", } }
